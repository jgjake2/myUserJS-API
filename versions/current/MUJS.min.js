...
// ==/UserScript==
if(typeof unsafeWindow==="undefined")unsafeWindow=window;MUJS=function(arg1,arg2){return MUJS.init.apply(MUJS,Array.prototype.slice.call(arguments,0))};Object.defineProperties(MUJS,{"version":{configurable:false,get:function(){return"0.0.13"}},"build_time":{configurable:false,get:function(){return"1418442129000"}},"fn":{value:MUJS.__proto__},"API":{value:function(){return MUJS.API.EvalCommand.apply(MUJS.API,Array.prototype.slice.call(arguments,0))}}});MUJS["API"]["fn"]=MUJS["API"].__proto__;
(function(MUJS,$){var mujsAPILoadStart=-1;var mujsAPILoadEnd=-1;if(typeof unsafeWindow.performance!=="undefined"&&typeof unsafeWindow.performance.timing!=="undefined")mujsAPILoadStart=unsafeWindow.performance.now();MUJS.fn.init=function(){var args=Array.prototype.slice.call(arguments,0);try{if(args.length>0)if(typeof args[0]==="string"){if(args[0]=="get")return MUJS.config(args[1]);if(args[0]=="set")return MUJS.config(args[1],args[2])}else if(typeof args[0]==="object")if(typeof args[0]["GM_info"]!==
"undefined"||typeof args[0]["ginfo"]!=="undefined")return MUJS.setScriptInfo.apply(MUJS,args)}catch(e){}};if(console)console.log("Loading MUJS API v"+MUJS.version+" - "+(new Date(parseInt(MUJS.build_time))).toString());var URLBuilder=function(input){var r={protocol:"http:",hostname:"",pathname:"",args:[],setHostname:function(str){var parser=document.createElement("a");if(!/https?\:\/\//i.test(str))str="http://"+str;parser.href=str;this.hostname=parser.hostname;this.protocol=parser.protocol},setPath:function(str){if(str[0]!=
"/")str="/"+str;this.pathname=str},addArg:function(key,value){this.args.push([key,value])},addArgs:function(args){for(var i=0;i<args.length;i++)this.addArg(args[i])},buildArgs:function(){var argStr="";var argsArr=[];for(var i=0;i<this.args.length;i++)argsArr.push(this.args[i].join("="));return argsArr.join("&")},toString:function(){return this.protocol+"//"+this.hostname+this.pathname+"?"+this.buildArgs()}};if(typeof input!=="undefined")r.setHostname(input);return r};function mExportFunction(func,
scope,args){if(typeof exportFunction!=="undefined")exportFunction(func,scope,args);else{var name;if(typeof args==="undefined")args={};if(typeof args.defineAs!=="undefined")name=args.defineAs;else if(typeof func==="function"&&func.name!="")name=func.name;scope[name||""]=func}}var mCloneInto=this.mCloneInto=function(obj,scope,args){if(typeof cloneInto!=="undefined")return cloneInto(obj,scope,args);else return obj};function Object_SearchForKey(str){var names=str.split(".");var tmp=this;for(var i=0;i<
names.length;i++){if(typeof tmp[names[i]]==="undefined")return undefined;tmp=tmp[names[i]]}return tmp}function Object_setKeyValue(str,val,force){var names=str.split(".");var tmp=this;for(var index=0;index<names.length-1;index++){if(typeof tmp[names[index]]==="undefined")if(force)tmp[names[index]]=new Object;else return;tmp=tmp[names[index]]}tmp[names[names.length-1]]=val}function RealTypeOf(obj){try{if(typeof obj==="undefined")return"undefined";if(typeof obj==="number"&&isNaN(obj)==true)return"nan";
if(typeof obj==="object"){if(obj===null)return"null";if(obj.constructor==={}.constructor)return"map";if(obj.constructor===(new Array).constructor)return"array";if(obj.constructor===(new Date).constructor){if(isNaN(obj.getTime()))return"invaliddate";return"date"}if(obj.constructor===(new RegExp).constructor)return"regex";return Object.prototype.toString.call(obj).replace(/^\[object |\]$/g,"").toLowerCase()}return typeof obj}catch(e){return typeof obj}}function clone(obj){if(obj==null||typeof obj!=
"object")return obj;var temp;try{temp=obj.constructor()}catch(e){switch(RealTypeOf(obj)){case "number":temp=(new Number).constructor();break;case "array":temp=(new Array).constructor();break;case "map":temp={}.constructor();break;default:temp=obj;break}}for(var key in obj)if(obj.hasOwnProperty(key))temp[key]=clone(obj[key]);return temp}function mergeObjects(obj1,obj2){if(obj2==null||["string","number","date","regex","undefined"].indexOf(RealTypeOf(obj2))!=-1)return clone(obj2);var objOut=clone(obj1);
for(var key in obj2)if(typeof objOut[key]==="undefined"||obj2[key]==null||typeof obj2[key]!="object")objOut[key]=clone(obj2[key]);else if(objOut[key]==null||RealTypeOf(objOut[key])=="array"||typeof objOut[key]!="object")objOut[key]=clone(obj2[key]);else objOut[key]=mergeObjects(objOut[key],obj2[key]);return objOut}function merge(){var args=Array.prototype.slice.call(arguments);if(args.length==0)return undefined;if(args.length==1)return args[0];var tmp=args[0];for(var i=1;i<args.length;i++)tmp=mergeObjects(tmp,
args[i]);return tmp}var props={SearchForKey:{value:Object_SearchForKey,enumerable:false},setKeyValue:{value:Object_setKeyValue,enumerable:false}};var performance=new function(){var getPerformancePtr=function(){return typeof unsafeWindow.performance!=="undefined"&&typeof unsafeWindow.performance.timing!=="undefined"?unsafeWindow.performance:undefined};var _performancePtr=undefined;this.__defineGetter__("available",function(){return typeof this.performancePtr==="undefined"?false:true});this.__defineGetter__("now",
function(){return this.performancePtr.now()});this.__defineGetter__("performancePtr",function(){if(typeof _performancePtr==="undefined")_performancePtr=getPerformancePtr();return _performancePtr});this.get=function(str){if(typeof this.performancePtr==="undefined")return undefined;var names=str.split(".");var tmp=this.performancePtr;for(var i=0;i<names.length;i++){if(typeof tmp[names[i]]==="undefined")return undefined;tmp=tmp[names[i]]}return tmp};this.getAllTiming=function(ignore){if(typeof ignore===
"undefined")ignore=[];var timingData=[];var ptr=this.performancePtr;for(var key in ptr.timing)if(!isNaN(ptr.timing[key])&&ignore.indexOf(key)==-1)timingData[key]=parseInt(ptr.timing[key]);return timingData}};MUJS["Config"]={"host":"http://myuserjs.org","scopeLock":typeof exportFunction==="undefined"?false:true,"secure":false,"script":{username:undefined,script_name:undefined},"Update":{"DOMTiming":false,"args":{},"updateVeriableName":"USMetaData","getType":"data","jQuery":false,"XMLHttpRequest":false,
"getStats":false,"sampleRate":100},"Error":{"autoReportErrors":false,"errorFilter":function(message,url,linenumber){return true}},"API":{"log":{"disabled_functions":[],"hidden_classes":[],"verbosity_level":5},"localStorage":{"storage_prefix":"MUJS_"}},"debug":false};Object.defineProperties(MUJS.Config,props);if(typeof unsafeWindow["MUJS_CONFIGURATION"]!=="undefined")MUJS.Config=merge(MUJS.Config,unsafeWindow["MUJS_CONFIGURATION"]);if(typeof unsafeWindow["MUJS_UPDATE_CONFIGURATION"]!=="undefined")MUJS.Config.Update=
merge(MUJS.Config.Update,unsafeWindow["MUJS_UPDATE_CONFIGURATION"]);Object.defineProperties(MUJS.Config,props);Object.defineProperty(MUJS,"config",{value:function(key,value){if(typeof value==="undefined")return MUJS.Config.SearchForKey(key);else return MUJS.Config.setKeyValue(key,value)},enumerable:false});MUJS["events"]={"e":{},"fired":{},"addEvent":function(name){this.e[name]=[];Object.defineProperty(MUJS,name,new function(propName){return{set:function(callback){MUJS["events"]["addListener"](propName,
callback)},enumerable:false}}(name))},"addListener":function(name,callback){if(typeof this.fired[name]==="undefined")this.e[name].push(callback);else callback.apply(MUJS,this.fired[name])},"fire":function(name){var args=Array.prototype.slice.call(arguments,1);this.fired[name]=args;if(typeof this.e[name]!=="undefined"){var putBack=[];while(i=this.e[name].pop())if(!i.apply(null,args))putBack.push(i);this.e[name]=putBack}}};MUJS["events"]["addEvent"]("onReady");MUJS["events"]["addEvent"]("onPageReady");
MUJS.fn.getScriptURLInfo=function(str){var patt=/myuserjs\.org\/script\/([^\/]+)\/([^\s]+)\.(user|meta|metajs|data)\.js/i;if(patt.test(str)){var matches=patt.exec(str);return{username:matches[1],script_name:matches[2],get_type:matches[3]}}return false};MUJS.fn.setScriptInfo=function(data){var callerScriptInfo;var output={};try{var genErrorVariable=genErrorFunction(genErrorArg)}catch(e){var tStack=MUJS.ERROR.parseStack(e.stack.toString());if(tStack.length>0)for(var i=tStack.length-1;i>=0;i--)if(tStack[i].fileName!=
""&&tStack[i].fileExt.toLowerCase()=="user.js"){callerScriptInfo=tStack[i];output.userscript_file_name=callerScriptInfo.fileName;output.userscript_file_path=callerScriptInfo.fullFileName;break}}try{var tGM_info;if(typeof data.GM_info!=="undefined")tGM_info=data.GM_info;else if(typeof data.ginfo!=="undefined")tGM_info=data.ginfo;if(typeof tGM_info!=="undefined"){if(typeof tGM_info.script!=="undefined")for(var key in tGM_info.script)if(typeof output[key]==="undefined")output[key]=tGM_info.script[key];
if(typeof tGM_info.uuid!=="undefined")output["gmUUID"]=tGM_info.uuid;else if(typeof tGM_info.script.uuid!=="undefined")output["gmUUID"]=tGM_info.script.uuid;if(typeof tGM_info.scriptHandler!=="undefined")if(tGM_info.scriptHandler.toLowerCase()=="tampermonkey"){output.script_handler="Tampermonkey";output.script_handler_version=tGM_info.version}else{if(tGM_info.scriptHandler.toLowerCase()=="greasemonkey"){output.script_handler="Greasemonkey";output.script_handler_version=tGM_info.version}}else if(data.has_GM_info){output.script_handler=
"Greasemonkey";output.script_handler_version=tGM_info.version}else if(data.has_GM_getMetadata)output.script_handler="Scriptish";var pMetaData=MUJS.API.ParseMetaData(tGM_info.scriptMetaStr);var urlInfo;if(typeof pMetaData["downloadURL"]!=="undefined"&&(urlInfo=MUJS.getScriptURLInfo(pMetaData["downloadURL"]))||typeof pMetaData["updateURL"]!=="undefined"&&(urlInfo=MUJS.getScriptURLInfo(pMetaData["updateURL"]))||typeof pMetaData["MUJSdownloadURL"]!=="undefined"&&(urlInfo=MUJS.getScriptURLInfo(pMetaData["MUJSdownloadURL"]))||
typeof pMetaData["MUJSupdateURL"]!=="undefined"&&(urlInfo=MUJS.getScriptURLInfo(pMetaData["MUJSupdateURL"]))){MUJS("set","script.username",urlInfo.username);MUJS("set","script.script_name",urlInfo.script_name);if(["meta","metajs","data"].indexOf(urlInfo.get_type.toLowerCase())!=-1)MUJS("set","script.script_name",urlInfo.script_name)}else{if(typeof pMetaData["MUJSusername"]!=="undefined")MUJS("set","script.username",pMetaData["MUJSusername"]);else if(typeof pMetaData["MUJS_username"]!=="undefined")MUJS("set",
"script.username",pMetaData["MUJS_username"]);if(typeof pMetaData["MUJSscriptname"]!=="undefined")MUJS("set","script.username",pMetaData["MUJSscriptname"]);else if(typeof pMetaData["MUJS_script_name"]!=="undefined")MUJS("set","script.username",pMetaData["MUJS_script_name"])}}}catch(e){}Object.defineProperty(MUJS.Config.Update,"script_info",{value:Object.freeze(output),writable:false,enumerable:true,configurable:false});return Object.freeze(output)};MUJS.fn.getScriptInfo=function(data){if(typeof MUJS.config("Update.script_info")===
"undefined")MUJS.setScriptInfo(data);return MUJS.config("Update.script_info")};MUJS.API.EvalCommand=function(command,arg1,arg2){var args=Array.prototype.slice.call(arguments,1);switch(command.toLowerCase()){case "log":case "gm_log":return MUJS.API.MUJS_Log.Log.apply(MUJS.API.MUJS_Log,args);break;case "debug":return MUJS.API.MUJS_Log.Debug.apply(MUJS.API.MUJS_Log,args);break;case "info":return MUJS.API.MUJS_Log.Info.apply(MUJS.API.MUJS_Log,args);break;case "warning":return MUJS.API.MUJS_Log.Warning.apply(MUJS.API.MUJS_Log,
args);break;case "startgroup":case "endgroup":case "groupcollapsed":case "time":case "timeend":case "timestamp":case "assert":case "trace":case "clear":case "count":case "table":return MUJS.API.MUJS_Log[command].apply(MUJS.API.MUJS_Log,args);break;case "style":case "addstyle":case "gm_addstyle":return MUJS.API.addStyle.apply(MUJS.API,args);break;case "script":case "addscript":case "gm_addscript":return MUJS.API.addScript.apply(MUJS.API,args);break;case "contenteval":return MUJS.API.contentEval.apply(MUJS.API,
args);break;case "getvalue":return MUJS.API.localStorage.getValue.apply(MUJS.API,args);break;case "setvalue":return MUJS.API.localStorage.setValue.apply(MUJS.API,args);break;case "deletevalue":return MUJS.API.localStorage.deleteValue.apply(MUJS.API,args);break;default:if(typeof MUJS.API[command]==="function")return MUJS.API[command].apply(MUJS.API,args);else if(typeof MUJS.API[command]!=="undefined")return MUJS.API[command];break}return undefined};var OUTPUT_TYPES={"ERROR":{level:1,value:"error"},
"WARNING":{level:2,value:"warn"},"INFO":{level:3,value:"info"},"LOG":{level:4,value:"log"},"DEBUG":{level:5,value:"debug"}};function isFirebug(ptr){if(typeof ptr==="undefined")return false;if(typeof ptr["timeStamp"]!=="undefined")return true;return false}function isConsole2(ptr){if(typeof ptr==="undefined")return false;if(!isFirebug(ptr))if(typeof ptr["dirxml"]!=="undefined")return true;return false}function isWebConsole(ptr){if(typeof ptr!=="undefined"&&typeof ptr["dirxml"]==="undefined")return true;
return false}function getFB(){if(isFirebug(this.console))return this.console;if(isFirebug(console))return console;if(isFirebug(window.console))return window.console;if(isFirebug(unsafeWindow.console))return unsafeWindow.console;if(isFirebug(unsafeWindow.window.console))return unsafeWindow.window.console;if(isFirebug(window.console))return window.console;return undefined}function getC2(){if(isConsole2(console))return console;if(isConsole2(this.console))return this.console;if(isConsole2(unsafeWindow.console))return unsafeWindow.console;
if(isConsole2(unsafeWindow.window.console))return unsafeWindow.window.console;if(isConsole2(window.console))return window.console;return undefined}function getWC(){if(isWebConsole(unsafeWindow.window.console))return unsafeWindow.window.console;if(isWebConsole(window.console))return window.console;return undefined}function MUJS_Log(fb_ptr,c2_ptr,wc_ptr){var tmp_fb_ptr=isFirebug(fb_ptr)?fb_ptr:undefined;var tmp_c2_ptr=isFirebug(c2_ptr)?c2_ptr:undefined;var tmp_wc_ptr=isFirebug(wc_ptr)?wc_ptr:undefined;
return{"Firebug_ptr":tmp_fb_ptr,"Console2_ptr":tmp_c2_ptr,"WebConsole_ptr":tmp_wc_ptr,"OUTPUT_TYPES":OUTPUT_TYPES,"updateFirebugPtr":function(new_ptr){if(typeof this["Firebug_ptr"]==="undefined"&&isFirebug(new_ptr))this.Firebug_ptr=new_ptr},"updateConsole2Ptr":function(new_ptr){if(typeof this["Console2_ptr"]==="undefined"&&isConsole2(new_ptr))this.Console2_ptr=new_ptr},"updateWebConsolePtr":function(new_ptr){if(typeof this["WebConsole_ptr"]==="undefined"&&isWebConsole(new_ptr))this.WebConsole_ptr=
new_ptr},"ConsoleCommand":function(command,value){try{var args=Array.prototype.slice.call(arguments,1);var safeArgs=mCloneInto(args,unsafeWindow,{cloneFunctions:true,wrapReflectors:true});if(typeof this.Firebug_ptr!=="undefined"&&typeof this.Firebug_ptr[command]!=="undefined")this.Firebug_ptr[command].apply(this.Firebug_ptr,safeArgs);if(typeof this.Console2_ptr!=="undefined"&&typeof this.Console2_ptr[command]!=="undefined")this.Console2_ptr[command].apply(this.Console2_ptr,safeArgs);if(typeof this.WebConsole_ptr!==
"undefined"&&typeof this.WebConsole_ptr[command]!=="undefined")this.WebConsole_ptr[command].apply(this.WebConsole_ptr,safeArgs);else if(typeof GM_log!=="undefined"&&isWebConsole(GM_log)&&["debug","log","info","warning","error"].indexOf(command.toLowerCase())!=-1)if(args.length==1)GM_log(args[0]);else if(args.length==2)GM_log(args[0],args[1]);else if(args.length==3)GM_log(args[0],args[1],args[2])}catch(e){console.log("ConsoleCommand Error! getUpdateData: ",e.name,e.fileName,e.lineNumber+":"+e.columnNumber);
console.log(e);console.trace(e)}},"outputMessage":function(output_type,str){if(output_type.level<=MUJS.config("API.log.verbosity_level"))this.ConsoleCommand.apply(this,[output_type.value].concat(Array.prototype.slice.call(arguments,1)))},"Error":function(category,x){this["outputMessage"].apply(this,[OUTPUT_TYPES["ERROR"],"C.CE Error ("+category+") - "+x.name+" - "+x.message+" in file <"+x.fileName+"> on line "+x.lineNumber])},"logError":this["Error"],"Warning":function(str){this["outputMessage"].apply(this,
[OUTPUT_TYPES["WARNING"]].concat(Array.prototype.slice.call(arguments,0)))},"Info":function(str){this["outputMessage"].apply(this,[OUTPUT_TYPES["INFO"]].concat(Array.prototype.slice.call(arguments,0)))},"Log":function(str){this["outputMessage"].apply(this,[OUTPUT_TYPES["LOG"]].concat(Array.prototype.slice.call(arguments,0)))},"Debug":function(str){this["outputMessage"](OUTPUT_TYPES["DEBUG"],str)},"table":function(data){this.ConsoleCommand("table",Array.prototype.slice.call(arguments,0))},"count":function(title){if(MUJS.config("API.log.disabled_functions").indexOf("count")==
...
["Debug","Log","Info","Warning"];MUJS.API.fn.MUJS_Log=new MUJS_Log(getFB(),getC2(),getWC());MUJS.fn.log=MUJS.API.MUJS_Log;MUJS.fn.log.UpdateAllPtrs=UpdateAllPtrs;for(var i=0;i<MUJS_Log_Functions.length;i++)MUJS.fn[MUJS_Log_Functions[i]]=MUJS.API.MUJS_Log[MUJS_Log_Functions[i]].bind(MUJS.API.MUJS_Log);MUJS.API.addStyle=function(css){if(typeof css!="undefined"&&css!="")if(GM_addStyle)GM_addStyle(css);else if(heads=document.getElementsByTagName("head")){var style=document.createElement("style");try{style.innerHTML=
css}catch(x){style.innerText=css}style.type="text/css";return heads[0].appendChild(style)}return null};MUJS.API.addScript=function(js,src,id,async){if(heads=document.getElementsByTagName("head")){var newScript=document.createElement("script");if(typeof js!="undefined"&&js!="")try{newScript.innerHTML=js}catch(x){newScript.innerText=js}if(typeof src!="undefined"&&src!="")try{newScript.src=src}catch(x){}if(typeof id!=="undefined")try{newScript.id=id}catch(x){}if(typeof async!=="undefined")newScript.async=
true;newScript.type="text/javascript";try{return heads[0].appendChild(newScript)}catch(x){}}return null};MUJS.API.contentEval=function(source){if("function"==typeof source)source="("+source+")();";var script=document.createElement("script");script.setAttribute("type","application/javascript");script.textContent=source;unsafeWindow.document.body.appendChild(script);unsafeWindow.document.body.removeChild(script)};Object.defineProperty(MUJS,"stor",{value:function(){return localStorage?localStorage:unsafeWindow.localStorage?
unsafeWindow.localStorage:window.localStorage}(),enumerable:false});MUJS.API.localStorage={};MUJS.API.localStorage.getValue=function(key,def){var name=MUJS.config("API.localStorage.storage_prefix")+key;if(MUJS.stor[name]){var value=MUJS.stor.getItem(name);return value}return def};MUJS.API.localStorage.setValue=function(key,value){var name=MUJS.config("API.localStorage.storage_prefix")+key;return MUJS.stor.setItem(name,value)};MUJS.API.localStorage.deleteValue=function(key,value){var name=MUJS.config("API.localStorage.storage_prefix")+
...
window.addEventListener("beforescriptexecute",BeforeScriptExec,true);function checkTimer(){var currentTime=performance.now;if(!MUJSRequire.Complete)BeforeScriptExec();else{RemoveEventListener_BeforeScriptExec();clearCheckTimer()}}function clearCheckTimer(){clearInterval(checkTimer)}setInterval(checkTimer,100);if(typeof GM_info!=="undefined")MUJS({"ginfo":GM_info,"has_GM_info":typeof GM_info!=="undefined"?true:false,"has_GM_getMetadata":typeof GM_getMetadata!=="undefined"?true:false});if(performance.available)mujsAPILoadEnd=
performance.now})(MUJS,typeof jQuery!=="undefined"?jQuery:undefined);unsafeWindow["MUJS"]=MUJS;
